...
if [[ "${install_prompt,,}" =~ ^(y|yes)$ ]]; then
	...
	if [[ "${auth_prompt,,}" =~ ^(y|yes)$ ]]; then
		...
		# Always point to the same DB the service will use:
		filebrowser config init -d "$DB_PATH" -a '0.0.0.0' -p "$PORT" &>/dev/null
		filebrowser config set  -d "$DB_PATH" --auth.method=noauth &>/dev/null
		# No users needed in no-auth mode
		msg_ok "No Authentication configured"
	else
		msg_info "Setting up default authentication"
		cd /usr/local/community-scripts
		filebrowser config init -d "$DB_PATH" -a '0.0.0.0' -p "$PORT" &>/dev/null
		filebrowser config set  -d "$DB_PATH" -a '0.0.0.0' -p "$PORT" &>/dev/null
